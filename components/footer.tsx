    ...
    <footer className="bg-[#0e3874] text-white w-10xl">
      ...
      <div className="mx-auto w-full grid grid-cols-1 gap-10 px-[5%] pt-14 pb-10 md:grid-cols-2 lg:grid-cols-13 lg:gap-8 lg:pt-16 ">
        ...
        <div className="lg:col-span-5">
          ...
          <p className="text-justify text-[0.95rem] leading-relaxed text-white/90 max-w-[600px] ">
            ...
            {socials.map(({ label, href, icon }) => (
              <li key={label}>
                <Link
                  href={href}
                  aria-label={label}
                  className="flex items-center gap-2 text-[1.0625rem] transition-colors hover:text-[#ffc425]"
                >
                  {icon}
                  <span>{label}</span>
                </Link>
      ...
      {/* Bottom bar */}
      <div className="px-[5%] flex flex-wrap items-center justify-between gap-3 border-t border-white/20 py-5 text-[0.95rem] font-semibold lg:py-6 lg:text-[1.0625rem]">
